...
resource "google_bigquery_table" "demo_view" {
  dataset_id      = google_bigquery_dataset.demo_dataset.dataset_id
  table_id        = "bq_demo_view"
  friendly_name   = "BigQuery Demo View"
  description     = "Demo view for BigQuery"
  expiration_time = null

  view {
    query = templatefile("${path.module}/view.sql", {
      project_id = var.project_id
      dataset_id = google_bigquery_dataset.demo_dataset.dataset_id
      table_id   = google_bigquery_table.demo_table.table_id
    })
